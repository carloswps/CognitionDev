...
function addBackground(result, primaryColor, secondaryColor, type, rotation) {
  ...
  switch (type) {
    ...
    case "gradientLinear":
      return `<rect fill="url(#backgroundLinear)" width="${width}" height="${height}" x="${x}" y="${y}" /><defs><linearGradient id="backgroundLinear" gradientTransform="rotate(${rotation} 0.5 0.5)"><stop stop-color="${xml$1(primaryColor)}"/><stop offset="1" stop-color="${xml$1(secondaryColor)}"/></linearGradient></defs>` + result.body;
  }
}
...
const create = ({ prng, options }) => {
  ...
  const initials2 = getInitials(prng.seed.trim()).slice(0, (_g = options.chars) !== null && _g !== void 0 ? _g : 2);
  const svg = [
    `<text x="50%" y="50%" font-family="${escapeXml(fontFamily)}" font-size="${fontSize}" font-weight="${fontWeight}" fill="${escapeXml(textColor)}" text-anchor="middle" dy="${(fontSize * 0.356).toFixed(3)}">${escapeXml(initials2)}</text>`
  ].join("");
  return {
    attributes: {
      viewBox: "0 0 100 100"
...
const initials = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  create,
  meta,
  schema
